...
- `act1-4_case_improved.js` — Extended case data
- `story/` — 38 `.json` + `.ink` files (added `peter_defense_test.json`)
...
### Weaknesses
- **Ink stories are severely truncated** — most JSON files contain only a single node with 2 choices that both go to "closing"
- No branching depth compared to 01
- No `unlocksEvidence` integration (dialogue doesn't reveal new evidence)
- Simplified compared to 01's rich narratives
...
### High Priority
1. **Restore Deep Ink Stories:** The JSON stories in `story/` are severely truncated. Port the full branching narratives from 01's story files.
2. **Fix Challenge Contradictions:** `npcSystem.challenge()` returns generic responses. Port the full `contradictions` data from 01 case files.
3. **Fix Pressure/Mood Sync:** 02's challenge updates `mesh.userData.npcState` but NPC system reads from `npcStates{}`. These are out of sync.
4. **Restore Evidence Unlocks from Dialogue:** 01's Ink stories trigger `unlocksEvidence`. 02's dialogue doesn't reveal new evidence items.
