        ...
        if (tilemapData?.background) {
            ...
            if (tilemapData.npcSpawns && tilemapData.npcSpawns.length > 0) {
                tilemapData.npcSpawns.forEach((spawn, i) => {
                    this.npcs.push({ id: spawn.id || `npc_${i}`, name: spawn.name || spawn.id || `Witness ${i + 1}`, x: spawn.x, y: spawn.y, location: spawn.location || '', animOffset: (Math.PI / 3) * i, avatar: "<img src='../assets/gfx/user-duotone.svg' class='icon-svg' loading='lazy'>" });
                });
            }
            ...
            if (tilemapData.enemyConfig) this.spawnEnemies(tilemapData.enemyConfig);
