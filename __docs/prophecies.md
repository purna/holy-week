...
> **✅ Fixed — `psalm_16_10` had no reveal path at all.** Unlike the Ezekiel case above, this one was a real gap, not a false positive: the prophecy record, its scroll evidence (`psalm_16_10_scroll`), and its `propheticRefs` entry on `galilean_apparition` all existed, but no `deductions` entry anywhere in `peter_restoration` set `revealsProphecy: "psalm_16_10"` — so the prophecy could never actually be completed in the Codex during play. The case's `deductions` object also contained an orphaned `"broiled_fish_remains+mary_encounter"` entry that referenced evidence IDs belonging to a different case (`resurrection` / `act4CaseA`) and could never fire. Fix applied directly in `act4_case.js`: the dead entry was replaced with a new `"galilean_apparition+psalm_16_10_scroll"` deduction (`link`, `isKey: true`) that sets `revealsProphecy: "psalm_16_10"`, and `truth.prophesyFulfilled` was corrected from `["Zechariah 13:7", "Ezekiel 34:11-16"]` to include all four of this case's prophecies. With this fix, every prophecy defined for `peter_restoration` — Zechariah 13:7, Ezekiel 34:11-16, Psalm 16:10, and Isaiah 53:10-11 — now has at least one working deduction path.
