...
export class UIManager {
  ...
  onLabAction(result) {
    ...
    if (result?.type === 'folder_verify' || result?.type === 'timeline_test' || result?.type === 'shredder_test') {
      ...
      this.cm.addScore(-1);
      if (result?.doubtDelta) this.cm.updateDoubt(result.doubtDelta);
  ...
  showCreditsModal() {
    this._setOverlayOpen("credits-modal", true);
    if (this.audio) this.audio.playHighStakes();
  }

  closeCreditsModal() {
    this._setOverlayOpen("credits-modal", false);
  }
  ...
  onChatAction(result) {
    ...
    if (result?.type === "talk_complete") { this.switchInvTab('scene'); this.switchInvTab('lab'); this.switchInvTab('casefile'); }
  }
}
